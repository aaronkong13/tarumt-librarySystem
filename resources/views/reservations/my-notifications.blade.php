    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 md:ml-64 relative">
            ...
            <header class="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-8 sticky top-0 z-10">
                ...
                <a href="{{ route('reservations.my') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
            ...
            <div class="p-8">
                ...
                @if(count($notifications) > 0)
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        @foreach($notifications as $notification)
                            <div class="bg-white rounded-xl shadow-sm border-2 {{ $notification['is_expired'] ? 'border-red-200' : 'border-green-200' }} p-6 hover:shadow-md transition-shadow relative overflow-hidden">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-shrink-0">
                                        @if($notification['book_cover'])
                                            <img src="data:image/jpeg;base64,{{ base64_encode($notification['book_cover']) }}" 
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="space-y-2 text-sm mb-4">
                                            <div class="flex items-center gap-2 text-gray-600">
                                                ...
                                                <span>Notified: {{ $notification['notified_at'] }}</span>
                                            </div>
                                            <div class="flex items-center gap-2 {{ $notification['is_expired'] ? 'text-red-600' : ($notification['remaining_days'] <= 1 ? 'text-orange-600' : 'text-gray-600') }}">
                                                <i class="fa-solid fa-calendar-xmark w-4"></i>
                                                <span>Expires: {{ $notification['expiry_date'] }}</span>
                                            </div>
                                        </div>
                                        ...
                                        @else
                                            <div class="p-3 {{ $notification['remaining_days'] <= 1 ? 'bg-orange-50 border-orange-200' : 'bg-green-50 border-green-200' }} border rounded-lg mb-3">
                                                <div class="flex items-center gap-2">
                                                    <i class="fa-solid fa-clock {{ $notification['remaining_days'] <= 1 ? 'text-orange-600' : 'text-green-600' }}"></i>
                                                    <p class="text-sm {{ $notification['remaining_days'] <= 1 ? 'text-orange-800' : 'text-green-800' }} font-medium">
                                                        {{ $notification['remaining_days'] }} day(s) remaining to collect
                                                    </p>
                                                </div>
                                            </div>

                                            <a href="{{ route('books.catalog') }}" class="block w-full text-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                ...
                @else
                    <div class="text-center py-16">
                        ...
                        <a href="{{ route('reservations.my') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                            <i class="fa-solid fa-bookmark"></i>

...
<body class="bg-gray-900/50 min-h-screen flex items-center justify-center p-4 backdrop-blur-sm">
    ...
    <div class="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden relative">
        ...
        <div class="flex justify-between items-center px-8 py-6 border-b border-gray-100">
            <h2 class="text-xl font-bold text-gray-900">Edit Book Details</h2>
            <a href="<?php echo e(route('books.index')); ?>" class="text-gray-400 hover:text-gray-600 transition-colors">
        ...
        <div class="p-8">
            ...
            <form action="<?php echo e(route('books.update', $book)); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="category" value="<?php echo e(old('category', $book->category)); ?>" 
                            class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3.5 outline-none transition-all" 
                            required>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            <select name="status" class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3.5 appearance-none cursor-pointer outline-none transition-all" required>
                                <?php $__currentLoopData = ['Available','Borrowed','Lost','Damaged']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($status); ?>" <?php echo e(old('status', $book->status) === $status ? 'selected' : ''); ?>><?php echo e($status); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    <label class="block mb-3 text-sm font-bold text-gray-900">Cover Image</label>
                    <div class="flex flex-col gap-4">
                        <?php if($book->cover_image): ?>
                            <div>
                                <p class="text-xs text-gray-500 mb-2">Current Cover:</p>
                                <img src="data:image/jpeg;base64,<?php echo e(base64_encode($book->cover_image)); ?>" alt="Current Cover" class="h-24 w-16 object-cover rounded-lg border border-gray-200 shadow-sm">
                            </div>
                        <?php endif; ?>

                        <div>
                            <p class="text-xs text-gray-500 mb-2">Upload New Cover (Optional):</p>
                            <label class="flex flex-col items-center justify-center w-full h-24 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-white hover:bg-gray-50 transition-colors">
                        ...
                        <div id="previewContainer" class="hidden">
                            <p class="text-xs text-gray-500 mb-2">New Preview:</p>
                            <img id="previewImage" src="" alt="Preview" class="h-24 w-16 object-cover rounded-lg border border-indigo-200 shadow-md bg-gray-100">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-50 mt-4">
                    <a href="<?php echo e(route('books.index')); ?>" class="px-6 py-3 text-sm font-bold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 focus:ring-4 focus:ring-gray-100 transition-all">
                        Cancel
                    </a>
...
</html><?php /**PATH C:\Users\Cheng\Documents\GitHub\tarumt-librarySystem\resources\views/books/edit.blade.php ENDPATH**/ ?>